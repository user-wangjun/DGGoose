...
> 交接版本：2026-08-11（Asia/Shanghai）
> 检查范围：代码结构、运行时资源、美术参考、音频、构建配置和交接风险；**本阶段代码测试、生产构建和内置浏览器验收已完成，证据保存在 `output/playwright/`。**
> 当前结论：**核心玩法代码和 M5 剧情分支已经形成；八个正式场景的运行时背景/布局、共享碰撞与互动层、前景排序、重复物件清理和多尺寸浏览器验收已完成；真实手机/平板、断网包、SFX 交付和发布包体仍未完成。**
...
- 主菜单、序章对话、八个正式场景和工厂移动流程已在内置浏览器走通关键入口与互动闭环。
- 本阶段代码测试、生产构建、正式场景美术/布局和浏览器冒烟已完成；真实手机/平板验收、断网包和 `npm run preview` 仍保留在后续 TODO。
...
1. 八个正式场景的背景、可行走区域、solid、互动点、出口和 foreground 已接入统一运行时布局；本阶段已用正常/碰撞调试画面逐场景核验。
2. 本阶段不生成新美术资源；场景内需要变化的角色/NPC 使用已有透明运行时资源，三视图/独立动作帧的后续生产仍不属于本阶段完成项。
3. `vite.config.js` 只复制 BGM 到生产 `dist/assets/audio/`，SFX WAV 没有被复制；发布构建中的 SFX 请求会失败并静默降级。
4. `src/config.js` 中五个 NPC portrait 已改为 Vite 可追踪的本地立绘 URL，对应文件已存在并接入对话 UI；场景内透明 NPC 动作资源仍未完成。
5. 资源体积超过项目规则：当前 `dist/` 约 **11.28 MB**，而 `PROJECT_RULES.md` 目标是总资源 `<5 MB`；源 `assets/` 约 **32.31 MB**。
6. `docs/plans/2026-08-08-topdown-scene-revision.md` 已确认俯视改造方向；八个正式场景均已完成共享地图/碰撞/互动/前景层落地，烧鹅店另完成障碍感知巡逻导航。
7. 尚未完成真实手机/平板横屏、触摸摇杆、全流程、断网包和 `npm run preview` 的最终验收。
8. `favicon.ico` 404 已通过移除无效引用解决；本阶段浏览器控制台验收为 0 error、0 warning。
...
- 篮球馆：俯视地图入口 → 拖拽投篮/抛物线 → 5 个固定篮筐平台小关卡 → 进球/投失反馈、投失自动复位、失败重试/领取纪念。
...
- `assets/characters/npcs/npc_coach_dialogue.png`
- `assets/characters/npcs/npc_farmer_lychee_dialogue.png`
- `assets/characters/npcs/npc_roast_goose_shop_owner_dialogue.png`
- `assets/characters/npcs/npc_dgut_senior_dialogue.png`
- `assets/characters/npcs/npc_dgut_senior_female_dialogue.png`
  - 五张 NPC 对话立绘已接入 `src/config.js` 的 `PORTRAIT_MAP`。
  - 复核标记：`dialogue-portrait-reviewed-2026-08-08`；逐张目视检查身份、服装、构图、单角色和学生校徽限制。
  - 仅覆盖对话 UI；不代表场景 NPC 色块、透明动作帧和工人剪影已完成。
...
- [x] **第一章“篮球馆”正式场景接入**：正式背景、归一化碰撞、教练/篮筐互动区、移动滑动和投篮入口已完成；已通过全量测试、生产构建和多尺寸浏览器验收。
- [x] **篮球馆投篮规则同步**：正式运行时使用拖拽瞄准、松手发射；每关切换固定篮筐和平台布局；有效物理开口约为 `±17px`，侧视横穿篮圈时下方最多容许 `16px`、上方仅保留 `2px` 离散误差；投失落地后自动回到当前小关卡起点；出手时播放莞小鹅 `throw` 六帧动作图集，动作结束回到待机初始帧。
- [ ] **篮球馆完整五球浏览器验收**：当前已验证投篮成功、Level 1 → Level 2、投失后自动复位；仍需补一次从 0 / 5 到 5 / 5 的完整通关证据。
- [x] **S2-S3 全场景验收**：序章、篮球馆自由移动/投篮、荔枝园、烧鹅店、工业园区、DGUT 校园、松山湖均完成内置浏览器关键流程检查；烧鹅店完成开场对话、30 秒巡逻、躲藏、发现/被抓/复位、偷尝和出口流程。

- [ ] **建立接手基线**：执行并记录 `npm install`、`npm test`、`npm run build`、`npm run preview`；再走一遍主菜单、序章、章节选择、存档、设置、分支和结局入口。
- [ ] **修正运行时资源引用**：让 `vite.config.js` 复制全部 SFX WAV；清理 `favicon.ico` 404 和其他资源 404。
  - [x] NPC portrait 路径已修正并接入五张已复核立绘。
- [x] **完成共享俯视地图基础**：已建立背景层、可行走/碰撞层、前景层、互动点和出口的数据结构，并跑通荔枝园“婆婆对话 → 摘取 → 交付 → 出口”闭环。
...
- [ ] **当前优先交付：印记与转场**：先完成 7 枚印记图标（已获得/灰态）和羽毛/鹅掌形转场遮罩，并接入章节地图、收集反馈和场景切换；完成后再进入大批量场景/NPC 美术。
- [x] **正式场景背景**：八个正式场景均已接入运行时背景，明确可行走区域、主地标、互动点、出口和前景遮挡；未把带设定图背景的候选图直接当运行时素材。
  - [x] 第一章“篮球馆”：已接入正式 16:9 背景，并完成碰撞、互动点、投篮入口和响应式画布验收。
  - [x] 序章、荔枝园、烧鹅店、工业园区、DGUT 校园、松山湖：已完成正常画面、碰撞调试画面和关键互动浏览器验收。
- [ ] **NPC 与莞小鹅动作**：将三视图/独立动作帧转成透明运行时资源，统一比例、脚底线、朝向和安全边距；补齐 NPC 头像、老板巡逻/被抓和莞小鹅 `interact`、`eat`、`scared`、`hide`、`caught`、`celebrate`、`sit`；篮球馆 `throw` 动作已接入。
- [ ] **场景物件与反馈**：补齐篮架/篮球、荔枝树/栅栏、桌底/木桶/门帘、安检门/无人机/齿轮、校园物件、松山湖长椅/芦苇，以及进球、收集、翻墙、被抓、结局和转场反馈。
- [ ] **UI 与结局 CG**：补齐章节地图节点、对话框、虚拟摇杆、设置面板和六套正式 Ending CG；印记和转场按上一项优先处理。
- [x] **BGM**：8 首 BGM 文件已存在并在 TODO 页面标记完成；后续只做实际播放、循环、音量和授权复核，不再作为 BGM 制作 TODO。
- [ ] **SFX 与资源交付**：试听确认 SFX 的情绪、音量、命名和授权；补缺失 SFX；清理重复/调试图；记录来源、生成日期、版本和 SHA-256。
...
- P1 未开始：正式场景背景、莞小鹅剧情动画、印记图标、烧鹅店老板动画；当前优先先做印记图标和转场遮罩。
...
- P0 只有核心序列帧标记为已完成。
- P1/P2/P3 视觉资源均标记为未开始；当前优先先做印记图标和转场遮罩。
- 页面本身定义了 S1 场景样板生成规则和以下提示词，已复制到本交接文档第 9 节。

### `art-audio-todo.html`

- 序章、篮球馆、荔枝园、烧鹅店、工业园区、DGUT BGM 标记为完成。
- 8 首 BGM 统一按“已完成”处理；后续只保留实际播放、循环、音量和授权复核，不再作为制作 TODO。
