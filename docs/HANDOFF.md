...
> 当前 TODO 唯一入口：[`docs/TODO.md`](./TODO.md)。本文后面的 P0/P1/P2、音频清单和外部 TODO 页面只保留为审计依据、参考文件和提示词，不作为新的任务分类。
...
> 执行顺序和任务归类以 [`docs/TODO.md`](./TODO.md) 为准；本节保留原始风险证据，便于追溯。

### P0：发布前必须处理

| 项目 | 现状 | 下一步验收 |
| --- | --- | --- |
| 正式场景背景 | 8 个模板/候选存在，但运行时仍使用程序化色块和几何图形 | 每个可玩章节至少接入一张符合构图的背景，并完成 16:9 视觉检查 |
| 俯视地图改造 | 方向文档已确认，实际共享地图层/碰撞层/前景层尚未完成 | 先以荔枝园闭环：婆婆对话 → 移动摘取 → 回婆婆交付 → 出口推进 |
| NPC 运行时资源 | 三视图存在，透明裁切、比例、脚底线、动作接入未完成 | 对话头像、场景角色、老板巡逻/被抓各完成一次真实运行时接入 |
| SFX 生产构建 | `vite.config.js` 只复制 8 个 BGM，不复制 WAV SFX | 构建后检查 `dist/assets/audio/` 包含所有运行时 SFX，并断网播放验证 |
| NPC portrait 路径 | `PORTRAIT_MAP` 指向多个不存在的 `coach_* / grandma_* / boss_* / senior_*` 文件 | 补资源或改成真实路径；打开每章对话检查无 404/空头像 |
| 资源授权 | PRD 仍将 IP 授权、音频版权列为待确认 | 明确莞小鹅 IP、场景素材、BGM/SFX 的可发布授权记录 |

### P1：品质与验收必须补齐

- 莞小鹅剧情/玩法动作：`interact`、`eat`、`throw`、`scared`、`hide`、`caught`、`celebrate`、`sit`。
- 烧鹅店老板 `boss_patrol` / `boss_caught` 的透明运行时动画和朝向。
- 7/12 印记图标、灰态、收集闪光和章节地图视觉资源。
- 场景物件：篮架/球、荔枝树/栅栏、桌底/木桶/门帘、安检门/无人机/齿轮、校园物件、松山湖长椅/芦苇。
- 对话框、虚拟摇杆、设置面板、章节地图节点、转场遮罩的正式 UI 美术。
- 终章六套 Ending CG 的正式插画；当前 `EndingCG.js` 是“场景定格 + 文案 + 色调”的首版实现。
- 手机/平板真实触控、safe-area、竖屏遮罩、单拇指全流程和低帧率降级实测。

### P2：可在发布后继续优化

- 包体从约 11.28 MB 压缩到 `<5 MB`：优先处理 BGM、菜单背景、头像和重复 JPG/PNG。
- 删除或迁移重复/调试资源：JPG 与 PNG 双份图集、`dummy-non-existing-folder/`、临时截图。
- 补 `favicon.ico` 或移除 HTML 引用。
- 更新 `docs/checklist.md` 中过期的 335/388 测试数字；接收者完成首轮测试后再填实际数量。
- 为资源添加版权、来源、生成日期、版本和 SHA-256 记录。
...
> 当前执行以 [`docs/TODO.md`](./TODO.md) 的“美术”部分为准。
...
> 音频任务已归入 [`docs/TODO.md`](./TODO.md) 的“美术”部分；本节只保留生成方向和历史状态。
