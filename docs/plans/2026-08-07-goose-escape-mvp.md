...
- **操作**: 创建 + TDD
- **要点**: 蓄力（65 点/秒，松开出手）；抛物线 `vy += 430*dt`、初速 `vx=120+p*3.1, vy=-(210+p*4.0)`；篮筐上下浮动（10%–34% 高）；进球判定（下落穿越篮圈平面且 |Δx|<36）；60s 倒计时、目标 5 球；失败/过关结算；过关发放印记"篮球徽章"+ 教练台词。
- **验证**: 测试：物理位移、边界、进球判定（命中/擦边/未中）、计时归零；手动：横屏触控按钮蓄力出手，开发期可用空格回退。
