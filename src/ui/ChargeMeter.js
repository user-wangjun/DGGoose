 ...
 *
 * 纯 DOM 组件，不使用 Canvas。
 * 提供水平进度条（绿→黄→红渐变）与蓄力按钮（触屏支持）。
 * 按住按钮时蓄力增长，松开时触发出手回调。
 *
 * 交互方式：
 * - 触控主路径：按住蓄力按钮 → 松开出手
 * - 开发回退：由场景层监听空格键 keydown/keyup，调用 onChargeStart/onChargeRelease
 */
...
export class ChargeMeter {
  ...
  mount(container) {
    ...
    this.labelElement.textContent = '蓄力';
    ...
    this.buttonElement.textContent = '按住蓄力';
